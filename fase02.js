 ...
 class fase02 extends Phaser.Scene {
    ...
    moverInimigo2(inimigo2) {
        if (inimigo2.x < inimigo2.minX || inimigo2.x > inimigo2.maxX) {
            inimigo2.speed *= -1
        }
...
//função
    pegaMoedas(jogador, moeda) {
        ...
        this.pontos++
        this.setPontos()
        if(this.grupo_m_coletaveis.countActive() <=0){
        this.musica.stop()
        this.scene.start('nivelVencido')
        this.scene.start('inicio', {pts: this.pontos})
         }}
    
//funçãof
   dano(jogador, inimigo, inimigo2){ //callback recebe como parametro os dois que passa antes
   ...
   init(fase01){ // quando de fase ele continua com a mesma pontuação
        this.pontos = fase01.pts
    ...
    create() {
        ...
        this.grupo_m_coletaveis = this.physics.add.group({
            ...
            setXY: {
                x: 15,
                y: -50,
                stepX: 200 // de tanto em tanto ele caiu de cima, pq da fisica
            }
...
//NÃO DEU CERTO COM O MEU, FICOU BUGADO
          ...
          this.grupo_inimigos = this.physics.add.group({
                key: 'inimigo',
                repeat: 0, //quantidade de inimigos que vai ser num espaço de 450 ?
                setXY: {
                x: Phaser.Math.Between (250,900),//aparecer aleatoriamente
                    y: 0,
                    stepX: 800 //- o espaço onde vai aparecer outro inimigo, igual moeda la
            }
      //gera numeros infinitos numeros entre 0 e 1, com if ternário, velocidade com o que o inimigo/objeto vai se mover
...
.setVelocity(Math.random()>0.5? -200:200, 50)  
  ...
  this.grupo_inimigo2 = this.physics.add.group({  
    allowGravity: false,
    immovable: true // bloqueia o objeto
  })
  
this.moveInimigo2 = this.grupo_inimigo2.create(1500, 150, 'inimigo2') 
.setScale(1, 1)// tamanho plataforma
.setOrigin(0, 1)
.refreshBody()

this.moveInimigo2.speed = 2//são variaveis, prof criou, não é o phaser que criou, o min e o mx tb
this.moveInimigo2.minX = 1500
this.moveInimigo2.maxX = 1900
     ...
     this.physics.add.collider(this.grupo_m_coletaveis, this.grupo_esteira)
      ...
      this.physics.add.collider(this.grupo_inimigo2, this.grupo_plataforma)// NOVO 
      this.physics.add.collider(this.grupo_inimigo2, this.grupo_m_plataforma, this.moveObjects)
      this.physics.add.collider(this.grupo_inimigo2, this.grupo_mv_plataforma, this.moveObjectsY)
      this.physics.add.collider(this.grupo_inimigo2, this.grupo_esteira, this.moveObjects)


        this.physics.add.collider(this.jogador, this.grupo_inimigos, this.dano, null, this)
        this.physics.add.collider(this.jogador, this.grupo_inimigo2, this.dano, null, this)
    ...
    update() {
        ...
        this.grupo_inimigos.children.iterate(
            (inimigo)=> {
                ...
                inimigo.setScale(3)//opcional
            }
        )

        this.grupo_inimigo2.children.iterate(
            (inimigo2)=> {
                inimigo2.setCollideWorldBounds(true)
                inimigo2.setBounce(0)//fica pulando para la e pra ca
                inimigo2.setScale(0.5)//opcional
            }
        )
        ...
        this.grupo_inimigo2.children.iterate(///TEM QUE FAZER A PARTE DE COLETAVEIS, PEGA COLETAVEIS, ETC
        //recebe método
        (p) => this.moverInimigo2(p)
    )
        ...
        this.grupo_m_coletaveis.children.iterate(
            (moeda)=>{
                //moeda.anims.play.stop() ISSO TAVA DANDO ERRO NA MINHA TELA DE GAME OVER
            })
    }//FIM DO IF
}//FIM DO UPDATE
