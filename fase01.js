 ...
 class fase01 extends Phaser.Scene {
   ...
   /* jumpMola(mola) {
        if (mola.y < mola.minY || mola.y > mola.maxY) {
            mola.speed *= -1
        }
        mola.y += mola.speed
    }
...
//função
    setPontos() {
        this.txtPontos.setText(
            this.pontos > 5 ?   // IF ternário com " ? "  e " : "
                `MOEDAS: ${this.pontos}` :
                `MOEDAS: 0${this.pontos}`
        )
    }
//função
pegaMoedas(jogador, moeda) {
    ...
    if(this.grupo_m_coletaveis.countActive() <=1){
   //this.somFundo.stop()
    this.scene.start('nivelVencido')
    //this.scene.start('fase02', {pts: this.pontos})
     }}
...
//função
   dano(jogador, inimigo){ //callback recebe como parametro os dois que passa antes
    ...
    create() {
        ...
        this.ceu = this.add.image(0, 0, 'ceu');// posição é 0, eixo x e y, inicio do jogo no canto que significa na onde meu jogo esta 
        this.ceu.displayWidth = 1055 //largura da imagem
        this.ceu.displayHeight = 600 //altura da imagem
        this.ceu.setOrigin(0, 0);//para descer aumenta o y e subir aumenta o x  

        this.ceu2 = this.add.image(1055, 0, 'ceu2');// posição é 0, eixo x e y, inicio do jogo no canto que significa na onde meu jogo esta 
        ...
        this.ceu3 = this.add.image(3359, 0, 'ceu3');// posição é 0, eixo x e y, inicio do jogo no canto que significa na onde meu jogo esta 
...
//móvel 2
  ...
  //plataforma móvel vertical 2
        ...
        this.grupo_plataforma.create(1200, 550, 'plataforma1')
            ...
            this.grupo_plataforma_mola = this.physics.add.group({
                allowGravity: false,
                immovable: true
            });
            this.grupo_mola = this.grupo_plataforma_mola.create(1700, 550, 'plataforma_mola')
            .setScale(0.2, 0.2)// tamanho plataforma
            .setOrigin(0, 1)
            .refreshBody()

        this.grupo_mola.speed = 2
        this.grupo_mola.minY = 500
        this.grupo_mola.maxY = 200
...
//NÃO DEU CERTO COM O MEU, FICOU BUGADO
          ...
          this.grupo_inimigos = this.physics.add.group({
                ...
                x: Phaser.Math.Between (250,900),//aparecer aleatoriamente
                    y: 0,
                    stepX: 1000 //- o espaço onde vai aparecer outro inimigo, igual moeda la
            }
        })
...
//QUANDO ATIVO ISSO AI, PIORA TUDO
        ...
        this.physics.add.collider(this.jogador, this.grupo_plataforma_mola, this.grupo_m_coletaveis)
     ...
     this.physics.add.collider(this.grupo_m_coletaveis, this.grupo_plataforma_mola)
      ...
      this.physics.add.collider(this.grupo_inimigos, this.grupo_plataforma_mola, this.moveObjects)

        this.physics.add.collider(this.jogador, this.grupo_inimigos, this.dano, null, this)

        /*this.physics.add.collider(this.jogador, this.grupo_mola, () => {
...
//MUSICAS E SONS
        ...
        this.musicaNivelVencido = this.sound.add('musicaNivelVencido')
    ...
    update() {
      ...
      /* this.grupo_plataforma_mola.children.iterate(///TEM QUE FAZER A PARTE DE COLETAVEIS, PEGA COLETAVEIS, ETC
        //recebe método
        (p) => this.jumpMola(p)
        )*/
